# origin: https://github.com/KevinLi-py/traceback-gui
import sys
import linecache
import textwrap
...
class VariableView(ScrolledTreeview):
    ...
    def add_variable(self, group, name, value):
        iid = self.insert(
            self.group_iids[group],
            "end",
            text=name,
            values=(_get_type(value), repr(value)),
        )
        ...
        try:
            ...
            if len(self.get_children(selection)) == 0:
                ...
                if isinstance(value, (tuple, list)) or (
                    hasattr(value, "__getitem__") and hasattr(value, "__len__")
                ):
                ...
                for k, v in items:
                    iid = self.insert(
                        selection, "end", text=k, values=(_get_type(v), repr(v))
                    )
...
class Page(ScrolledText):
    ...
    def write_code(self, filename, error_lineno):
        for lineno, code_line in _get_code(filename, error_lineno):
            self.write_text(
                "{:7d} ".format(lineno),
                "error-lineno" if lineno == error_lineno else "lineno",
            )
...
def _get_code(filename, error_lineno):
    ...
    line_length = len(max(code_lines, default="", key=len))
